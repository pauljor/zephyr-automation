...
10. Create the Bug issue in project `EI` per the spec: summary `[BUG] [<Category>] <Test Case.Name>` (category from step 5), the Markdown description built from steps 6–8 (Reason, then `**API**` line(s) if any from step 6, then Zephyr Test Case, then Zephyr Execution), `additional_fields` with `customfield_10360` (Test Name = the Test Case.Key) and `customfield_10359` (Test Status = FAILED, option id `10328`), and `transition: {"id": "8"}` to land directly in "BUG - Blocked by Defect".
